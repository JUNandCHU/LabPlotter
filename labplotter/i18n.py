...
KO = {
    ...
    "Auto OCR draft · review required": "자동 OCR 초안 · 검수 필요",
    ...
    "Open particle library…": "입자 라이브러리 열기…",
    "ZetaSizer particle library": "ZetaSizer 입자 라이브러리",
    "Particles included in the four plots": "4개 그래프에 포함된 입자",
    "Add from library…": "라이브러리에서 추가…",
    "Remove selected from plots": "선택 입자를 그래프에서 제거",
    "Add selected to plots": "선택 입자를 그래프에 추가",
    "Particle": "입자",
    "Batch": "배치",
    "Z-average": "Z-average",
    "Average zeta": "평균 zeta",
    "Average zeta potential": "평균 zeta 전위",
    "DLS n": "DLS 수",
    "Zeta n": "Zeta 수",
    "OCR n": "OCR 검수 수",
    "OCR status": "OCR 상태",
    ...
    "Peak value labels": "최대 피크값 표시",
    "Allow labels for multiple particles": "여러 입자에서도 피크값 표시",
    "DLS distribution": "DLS 분포",
    "Zeta distribution": "Zeta 분포",
    "Batch vs DLS Z-average": "배치별 DLS Z-average",
    "Batch vs average zeta potential": "배치별 평균 zeta 전위",
    "Batch comparison": "배치 비교",
    "Series colors": "데이터 색상",
    "Curve and bar colors": "곡선 및 막대 색상",
    "Color mode": "색상 적용 방식",
    "One color for all": "전체에 한 색상 적용",
    "Set colors by particle": "입자별 색상 지정",
    "Color for all {items}": "모든 {items}의 색상",
    "curves": "곡선",
    "bars": "막대",
    "Individual particle colors": "입자별 개별 색상",
    "Add particles to the plots to set individual colors.": "개별 색상을 지정하려면 먼저 플롯에 입자를 추가하세요.",
    "+ {count} more · see Series colors": "외 {count}개 · 데이터 색상에서 확인",
    "Summary and error bars": "대표값과 오차막대",
    "Central value": "대표값",
    "Median": "중앙값",
    "Show error bars": "오차막대 표시",
    "Editable batch names": "편집 가능한 배치 이름",
    "Source particle": "원본 입자 이름",
    "Batch name on X-axis": "X축 배치 이름",
    "Reset batch names to automatic JM labels": "배치 이름을 자동 JM 레이블로 복원",
    "Edit batch name…": "배치 이름 편집…",
    "Edit batch name": "배치 이름 편집",
    "Measurements and sources": "측정 및 원본",
    "Stored OCR data": "저장된 OCR 데이터",
    "Replicate": "반복",
    "Cell": "셀",
    "Status": "상태",
    "OCR field": "OCR 항목",
    ...
    "Stored {curves} replicate curves for {particles} particles and automatically read {tables} result tables.": "{particles}개 입자의 반복 측정 곡선 {curves}개를 저장하고 결과표 {tables}개를 자동으로 읽었습니다.",
    "Some OCR readings need review:": "일부 OCR 결과는 검토가 필요합니다:",
    "Importing and reading result tables": "가져오기 및 결과표 읽기",
    "Reading workbook data…": "통합문서 데이터를 읽는 중…",
    "Reading file {current}/{total}: {name}": "파일 읽는 중 {current}/{total}: {name}",
    "Reading file {current}/{total}: {name}\nSheet {sheet}/{sheets}: {sheet_name}": "파일 읽는 중 {current}/{total}: {name}\n시트 {sheet}/{sheets}: {sheet_name}",
    "Storing curves from file {current}/{total}: {name}": "곡선 저장 중 {current}/{total}: {name}",
    "OCR {current}/{total}: {file}\n{particle} · {kind} measurement {replicate}": "OCR 처리 중 {current}/{total}: {file}\n{particle} · {kind} 측정 {replicate}",
    "OCR {current}/{total} completed: {particle} · {kind} measurement {replicate}": "OCR 완료 {current}/{total}: {particle} · {kind} 측정 {replicate}",
    "Finalizing imported data…": "가져온 데이터를 마무리하는 중…",
    "Import a workbook, then select particles from the library": "통합문서를 가져온 뒤 라이브러리에서 입자를 선택하세요",
    "Add particles from the library": "라이브러리에서 입자를 추가하세요",
    "Automatic OCR values will appear here": "자동 OCR 값이 여기에 표시됩니다",
    "Result tables": "결과표",
    ...
    "Automatic OCR is stored on import. Open the selected measurement to review or correct it.": "가져올 때 자동 OCR 결과가 저장됩니다. 선택한 측정을 열어 검수하거나 수정할 수 있습니다.",
    "Review current OCR…": "현재 OCR 검수…",
    "Resetting the ZetaSizer particle library…": "ZetaSizer 입자 라이브러리를 초기화하는 중…",
    "A database reset patch must include database migration backup metadata.": "데이터베이스 초기화 패치에는 데이터베이스 백업 정보가 필요합니다.",
    # TEM particle-size analysis
    "Open TEM TIFF(s)…": "TEM TIFF 열기…",
    "Export particle CSV…": "입자 CSV 내보내기…",
    "Files are grouped by batch. Blank fields and exact duplicates are skipped automatically.": "파일은 배치별로 묶입니다. 빈 화면과 완전히 동일한 중복 파일은 자동으로 제외됩니다.",
    "Batch / image": "배치 / 이미지",
    "Magnification": "배율",
    "Calibration": "보정값",
    "Particle n": "입자 n",
    ...
    "Rename batch…": "배치 이름 변경…",
    "Reanalyze selected": "선택 이미지 재분석",
    "Extra images increase image and particle sampling, not the independent batch n.": "이미지를 추가하면 이미지 및 입자 표본 수가 늘지만 독립 배치 n이 늘어나는 것은 아닙니다.",
    "Size distribution": "크기 분포",
    "Segmentation review": "입자 분할 검수",
    "Equivalent particle diameter": "등가 입자 직경",
    "Probability density": "확률 밀도",
    "Detection overlay": "입자 검출 오버레이",
    "Selected image analysis": "선택 이미지 분석",
    "Batch name": "배치 이름",
    "Printed scale value (nm)": "표시된 스케일 값(nm)",
    "Detected bar length (px)": "검출된 스케일 바 길이(px)",
    "Minimum diameter (nm)": "최소 직경(nm)",
    "Maximum diameter (nm)": "최대 직경(nm)",
    "Minimum center distance (nm)": "최소 중심 간격(nm)",
    "Threshold factor": "분할 임계값 계수",
    "Exclude particles touching image borders": "이미지 경계에 닿은 입자 제외",
    "Force analysis of an auto-blank field": "자동 빈 화면도 강제로 분석",
    "Show detected-particle overlay": "검출 입자 오버레이 표시",
    "Apply and reanalyze image": "적용 후 이미지 재분석",
    "Select an image to review.": "검수할 이미지를 선택하세요.",
    "This is a screening estimate. Confirm the scale and detection overlay before reporting particle sizes.": "이 결과는 선별용 추정값입니다. 입자 크기를 보고하기 전에 스케일과 검출 오버레이를 확인하세요.",
    "TEM TIFF images": "TEM TIFF 이미지",
    "TEM particle analysis": "TEM 입자 분석",
    "Preparing TIFF analysis…": "TIFF 분석 준비 중…",
    "Analyzing TIFF {current}/{total}: {name}": "TIFF 분석 중 {current}/{total}: {name}",
    "Imported {images} unique TIFF images. Auto-skipped: {blank} blank, {duplicates} duplicate. Needs manual scale: {scale}.": "고유 TIFF 이미지 {images}개를 가져왔습니다. 자동 제외: 빈 화면 {blank}개, 중복 {duplicates}개. 수동 스케일 입력 필요: {scale}개.",
    "Errors:": "오류:",
    "{count} included images": "포함 이미지 {count}개",
    "{count} total images": "전체 이미지 {count}개",
    "Analyzed": "분석 완료",
    "Blank · skipped": "빈 화면 · 제외",
    "Needs scale": "스케일 필요",
    "Excluded": "제외됨",
    "Independent batches: {batches} · included images: {images} · detected particles: {particles} · blank images: {blank}": "독립 배치: {batches} · 포함 이미지: {images} · 검출 입자: {particles} · 빈 이미지: {blank}",
    "Detected particles: {count}": "검출 입자: {count}",
    "Segmented area: {percent:.1f}%": "분할된 면적: {percent:.1f}%",
    "Blank field automatically skipped.": "빈 화면으로 판정되어 자동 제외되었습니다.",
    "Enter the printed scale value and detected bar length, then reanalyze.": "표시된 스케일 값과 검출된 바 길이를 입력한 뒤 재분석하세요.",
    "Segmentation coverage is unusual; review the overlay and adjust the threshold.": "입자 분할 면적이 비정상적입니다. 오버레이를 검토하고 임계값을 조정하세요.",
    "Particle overlap is high; merged regions can overestimate diameter. Review the overlay and consider a higher magnification or stricter maximum diameter.": "입자 겹침이 많아 합쳐진 영역에서 직경이 과대 추정될 수 있습니다. 오버레이를 확인하고 더 높은 배율 이미지 또는 더 엄격한 최대 직경을 고려하세요.",
    "Few particles were detected; review the overlay and analysis settings.": "검출된 입자가 적습니다. 오버레이와 분석 설정을 검토하세요.",
    "particles": "개 입자",
    "Analysis settings must be numeric.": "분석 설정에는 숫자를 입력해야 합니다.",
    "Select one image first.": "먼저 이미지 하나를 선택하세요.",
    "Scale value and bar length must be positive.": "스케일 값과 바 길이는 양수여야 합니다.",
    "Rename batch": "배치 이름 변경",
    "Select one batch or image first.": "먼저 배치 또는 이미지 하나를 선택하세요.",
    "Delete TEM images": "TEM 이미지 삭제",
    "Delete {count} selected imported images and their particle-size results? This cannot be undone.": "선택한 가져오기 이미지 {count}개와 입자 크기 결과를 삭제할까요? 이 작업은 되돌릴 수 없습니다.",
    "Export particle CSV": "입자 CSV 내보내기",
    "No TEM results are available.": "내보낼 TEM 결과가 없습니다.",
    "CSV data": "CSV 데이터",
    "Open one or more TEM TIFF images": "하나 이상의 TEM TIFF 이미지를 여세요",
    # Custom formats
